...
if ($resolved) {
    ...
    if ($doIt) {
                    ...
                    if ($LASTEXITCODE -ne 0) {
                        ...
                        if ($LASTEXITCODE -ne 0) { Warn 'commit failed - see output above; the files are still in the worktree' }
                            ...
                            if ($LASTEXITCODE -ne 0) {
                                ...
                                Note "retry with:  git push -u origin $branch && gh pr create -R {{GITHUB_OWNER}}/{{CONTROL_PLANE_REPO}} --fill"
                            }
                            else {
                                ...
                                else { Warn "PR creation failed: $prOut"; Note 'open it by hand:  gh pr create -R {{GITHUB_OWNER}}/{{CONTROL_PLANE_REPO}} --fill' }
                            }
        ...
        if (-not $landed) {
            ...
            Note "  git push -u origin $branch && gh pr create -R {{GITHUB_OWNER}}/{{CONTROL_PLANE_REPO}} --fill"
        }
    }
    else {
