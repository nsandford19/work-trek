...
- generated-at: 2026-09-18T21:10:18Z
...
- Contents: 6 open issue(s), then 4 closed in the last 90 days, ascending by number.
...
| --- | --- | --- | --- |
| `Millers-IT/mpix-3` | #3407 | 2026-09-17 14:59 | Angular Gifts dropdown **and mobile menu** links + CMSKey pricing entries |
...
mpix-3 merge commit `bce170d2f`; deploying at the time of writing.
...
`/photo-gifts/framed-magnets`. Sitemap entries for both are committed in mpix-3
(`85c51efe1`).
...
**Squidex — the original blocker is cleared.** Both `PageProduct` entries are now
**published in prod** (`cms.mpix.com`), as of 2026-09-16 22:03 UTC, with
`categoryUrl: photo-gifts` and `url: framed-ornaments` / `framed-magnets`. That pair is
what the CMSKey lookup matches on, so pricing resolves rather than silently missing — the
open question about `framed_ornaments`' `categoryUrl` is answered. The `ProductDetail`
entries are no longer orphaned: nine published details hang off each page. Both
`DefProduct`s are published and their `syncMLProducts` resolve.

**Outstanding:**

- Naming diverged between tiers. mpix-3 labels the links **Framed Photo Magnets** and
  **Framed Photo Ornaments**; the two legacy tiers shipped "Framed Magnets" / "Framed
  Ornaments". PRs open to align them — `MillersProfessionalImaging/Mpix.Com` #560 and
  `Millers-IT/Mpix.Com.Core` #57 (branch `chore/framed-photo-link-labels` in both, display
  text + `title` attributes only; URLs and `New` badges untouched).
- `sku` is null and `syncYotpoMLProducts` is null on **both** `framed_magnets` and
  `framed_ornaments` `DefProduct`s — previously flagged for magnets only. The PDPs will
  render without reviews until these are set.
