#!/bin/bash
# Claude Code status line - canonical copy.
#
# Install on a machine with:  ./bin/mc.ps1 statusline
# which copies this file to ~/.claude/statusline-command.sh and points
# settings.json .statusLine at it. Edit THIS file, never the installed copy;
# see tooling/claude/README.md for the round trip.
#
# Reads the status line JSON payload on stdin, prints:
#   line 1  model | context % | dir (branch) | session length | effort
#   line 2+ 5-hour / weekly / extra-usage meters
# Needs bash and jq; curl is only used for the rate-limit fallback.
set -f
